...
class HMM:
    ...
    def getMatrix(self):
        self.prior = np.genfromtxt("hmmprior_1.txt", dtype=np.float64)
        # self.prior = np.genfromtxt(prior, dtype=np.float64)
        self.trans = np.genfromtxt("hmmtrans_1.txt", dtype=np.float64)
        # self.trans = np.genfromtxt(trans, dtype=np.float64)
        self.emit = np.genfromtxt("hmmemit_1.txt", dtype=np.float64)
    ...
    def getPredict(self):
        for i in range(0, len(self.X)):
            ...
            if i == 0:
                f = open("predictedtest_1.txt", "w")
                # f = open(predict, "w")
            else:
                f = open("predictedtest_1.txt", "a")
                # f = open(predict, "a")
