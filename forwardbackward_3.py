...
class HMM:
    def __init__(self):
        ...
        self.loglike = []  # Save all the log likelihood
        self.ave = 0  # Average Likelihood of All the Sequences

    def readIndex(self):
        id_w = open("fulldata/index_to_word.txt", 'r+').read().splitlines()
        # self.id_w = open(index_w, 'r+').read().splitlines()
        id_t = open("fulldata/index_to_tag.txt", 'r+').read().splitlines()
    ...
    def readFile(self):
        test = open("fulldata/testwords.txt", 'r+').read().splitlines()
    ...
    def getMatrix(self):
        self.prior = np.genfromtxt("hmmprior_3.txt", dtype=np.float64)
        # self.prior = np.genfromtxt(prior, dtype=np.float64)
        self.trans = np.genfromtxt("hmmtrans_3.txt", dtype=np.float64)
        # self.trans = np.genfromtxt(trans, dtype=np.float64)
        self.emit = np.genfromtxt("hmmemit_3.txt", dtype=np.float64)
    ...
    def getPredict(self):
        for i in range(0, len(self.X)):
            ...
            #
            # if i == 0:
            #     f = open("predictedtest_3.txt", "w")
            #     # f = open(predict, "w")
            # else:
            #     f = open("predictedtest_3.txt", "a")
            #     # f = open(predict, "a")
            #
            # f.write(sentence + '\n')
            # f.close()
            ...
            # print self.alpha[:, -1]
            log = np.log(np.sum(self.alpha[:, -1]))
            # print log
            self.loglike.append(log)
...
# print h.alpha
h.getPredict()
# print float(np.sum(h.alpha[:, -1]))/9
